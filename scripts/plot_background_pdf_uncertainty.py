...
def main() -> None:
    parser = ArgumentParser("plot_background_pdf_uncertainty.py")
    ...
    _ = parser.add_argument(
        ...
        action="store",
        type=Path,
        default="qcd_pdf_uncertainty.pdf",
    ...
    # Make plot
    canvas = ROOT.TCanvas("canvas", "p p -> j j cross sections", width, height)
    ...
    hr = canvas.DrawFrame(-1.5, x_min, num_pdfs + 0.5, x_max)
    hr.SetTitle("PDF impact on QCD 2 #rightarrow 2 decay cross section")
    ...
    for i in range(graph.GetN()):
        ...
        text.DrawText(x - 0.1, y + 0.15, pdf_name)

    latex = ROOT.TLatex()
    latex.SetTextSize(0.025)
    latex.SetTextAlign(12)
    latex.DrawLatexNDC(0.18, 0.2, "MadGraph 3.5.15")
    latex.DrawLatexNDC(0.18, 0.16, " p p #rightarrow j j")
